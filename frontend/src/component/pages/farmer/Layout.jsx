...
const CHAT_TYPE_INFO = {
  global:     { label: "Global Chat",    color: "#00a884", bg: "#d9fdd3" },
  farmers:    { label: "Farmers Chat",   color: "#34B7F1", bg: "#e8f5fe" },
  buyers:     { label: "Buyers Chat",    color: "#F2A93B", bg: "#fff1d6" },
...
function useChatUnread() {
  ...
  const fetchUnread = useCallback(async () => {
    ...
    if (!token) return;
    try {
      const res = await fetch(`${BASE_URL}/chat/unread-counts/`, {
...
export default function BuyerLayout() {
  ...
  const connectWebSocket = useCallback(() => {
    ...
    if (wsRef.current?.readyState === WebSocket.OPEN) wsRef.current.close(1000, "Reconnecting");
        ...
        if (data.event === "new_notification") {
          ...
          toast.info(
            <div onClick={() => handleNotificationClick(n)} style={{ cursor: "pointer" }}>
              ...
              <p style={{ fontSize: "12px", margin: "4px 0 0", color: "#666" }}>{n.description}</p>
            </div>,
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (chatMsgRef.current && !chatMsgRef.current.contains(e.target)) setChatMsgOpen(false);
    };
  ...
  const navItems = [
    ...
    { icon: <Wheat           size={17} />, label: t("nav.dataEntry",     { defaultValue: "Data Entry" }),     path: "/farmer/myStocks"                 },
    { icon: <FileSignature   size={17} />, label: t("nav.contracts",     { defaultValue: "Contracts" }),      path: "/farmer/contracts"             },
    // { icon: <ClipboardList   size={17} />, label: t("nav.standards",     { defaultValue: "Standards" }),      path: "/farmer/standards"             },
    { icon: <MessageSquare   size={17} />, label: t("nav.chats",         { defaultValue: "Chats" }),          path: "/farmer/chats"                 },
      ...
      <div style={{ display:"flex", minHeight:"100vh", background:"#f0f2f5" }}>
        ...
        <aside className={`sidebar ${sidebarOpen ? "mobile-open" : ""}`}>
          <div className="sidebar-brand">
            <div className="brand-icon"><Home size={20} color="#0f2718" /></div>
          ...
          <nav className="sidebar-nav">
            ...
            <div className="profile-actions-sidebar">
              ...
              {[
                { icon: <UserCircle size={17} />, label: t("nav.myProfile", { defaultValue: "My Profile" }) },
                { icon: <Settings   size={17} />, label: t("nav.settings",   { defaultValue: "Settings" }) },
              ].map((action, i) => (
                <button key={i} className="profile-action-sidebar"
          ...
          <header className="topbar">
            ...
            <div className="topbar-right">
                ...
                <div className={`dropdown-menu notif-menu ${chatMsgOpen ? "open" : ""}`}>
                  ...
                  <div className="notif-list">
                    {chatUnreadData.length === 0 ? (
                      ...
                      chatUnreadData.map(chat => {
                        const typeInfo = CHAT_TYPE_INFO[chat.chat_type] || CHAT_TYPE_INFO.one_on_one;
                        const ChatIcon = chat.chat_type === "one_on_one" ? MessageCircle
                          : chat.chat_type === "farmers" ? Users
                          : chat.chat_type === "buyers"  ? ShoppingBag
                          : MessageSquare;
                        return (
                          <div key={chat.chat_id} className="notif-item unread"
                            onClick={() => { setChatMsgOpen(false); navigate("/farmer/chats", { state: { openChatId: chat.chat_id } }); }}>
                  ...
                  <div className="notif-footer">
                    <button className="notif-footer-link" onClick={() => { setChatMsgOpen(false); navigate("/farmer/chats"); }}>
              ...
              {/* ── Notifications dropdown ─────────────────────────────────────────
                   ...
                   "View all notifications" navigates to /farmer/notificationss.
              ──────────────────────────────────────────────────────────────── */}
              <div className="dropdown-wrap" ref={notifRef}>
                ...
                <div className={`dropdown-menu notif-menu ${notifOpen ? "open" : ""}`}>
                  <div className="notif-header">
                    ...
                    {unreadCount > 0 && (
                      <button className="notif-mark-read" onClick={markAllAsRead} disabled={loading}>
                  ...
                  {/* ── Footer: always navigates to /farmer/notificationss ── */}
                  <div className="notif-footer">
                    <button
                      className="notif-footer-link"
                      onClick={() => { setNotifOpen(false); navigate("/farmer/notificationss"); }}
                    >
              ...
              <div className="dropdown-wrap" ref={langRef}>
                <button className={`lang-trigger ${langOpen ? "active" : ""}`}
                  onClick={() => { setLangOpen(v => !v); setProfileOpen(false); setNotifOpen(false); setChatMsgOpen(false); }}>
              ...
              <div className="dropdown-wrap" ref={profileRef}>
                <button className={`profile-trigger ${profileOpen ? "active" : ""}`}
                ...
                <div className={`dropdown-menu profile-menu ${profileOpen ? "open" : ""}`}>
                  ...
                  <div className="profile-actions">
                    ...
                    <button className="profile-action-item" onClick={() => { setProfileOpen(false); navigate("/farmer/chats"); }}>
