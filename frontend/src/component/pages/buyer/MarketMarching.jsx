    ...
    return (
        <div className="stats-overview">
            ...
            <SummaryCard
                ...
                color="#b76e0a"
                bgColor="#fff8e1"
                subtitle="Average match quality"
            />
            <SummaryCard
            ...
            <SummaryCard
                title={t('total_value')}
                value={formatCurrency(stats.total_potential_value || 0)}
...
function DistributionChart({ title, data, icon: Icon, type = 'product' }) {
    ...
    const dataArray = data && typeof data === 'object' && !Array.isArray(data)
...
export default function BuyerMarketMatching() {
    const { t } = useTranslation();
    ...
    const fetchMatches = useCallback(async (fetchFilters, page, size) => {
        ...
        try {
            ...
            const response = await apiClient.get(`/market-matching/buyer/matches/?${params}`, {
            ...
            if (response.data) {
                console.log('Matches data received:', {
                    ...
                    byProduct: response.data.statistics?.by_product,
                    byLocation: response.data.statistics?.by_location
                });
                ...
                // Handle statistics - check if statistics exists in response
                const statsData = response.data.statistics || {};
                setStats({
                    total_matches: statsData.total_matches || matchesData.length,
                    average_score: statsData.average_score || 0,
                    high_quality_matches: statsData.high_quality_matches || 0,
                    total_potential_value: statsData.total_potential_value || 0,
                    by_product: statsData.by_product || {},
                    by_location: statsData.by_location || {}
                });
            }
        } catch (error) {
        ...
        try {
            ...
            const response = await apiClient.get(`/chat/my-chats/`, {
                params: {
                    ...
                    page_size: 500 // Increase page size to find the chat
                }
            });
